// The soap bubbles' film.js as it stood before its colour table's columns were spaced by the
// square root of the cosine: kept as a specimen of columns spaced evenly in the cosine, which
// read the table too coarsely near grazing, where the faces' reflections change fastest.
// Only this note and one phrase of the comment below differ from that file.
...
export function colourTable() {
  ...
  for (let column = 0; column < columns; column += 1) {
    const cosAir = Math.max(1e-3, column / COSINE_STEPS);
    const cosWater = cosInside(cosAir);
...
export function lookUp({ table, rows, columns }, opd, cosAir) {
  ...
  const v = Math.min(columns - 1, Math.max(0, cosAir * COSINE_STEPS));
