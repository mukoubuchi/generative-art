/**
 * Voronoi Bloom as it stood at facea56, the commit before a06b223 repaired it. Frozen on
 * purpose: this is the fault a reader on a dense display actually saw, kept as a specimen
 * for the density check to be aimed at.
 *
 * Two things are wrong with it, and both are wanted here. `pixelDensity(1)` is called
 * before `createCanvas`, where p5 has nothing to set it on and quietly ignores it; and the
 * pixel loop walks a logical grid while indexing by `OUTPUT_WIDTH`. On a display of density
 * two the backing store comes out twice the size asked for, so the picture lands in the top
 * quarter of the canvas -- twice, side by side -- and the rest of the buffer is never
 * written to at all.
 *
 ...
 * to make room for it. A copy is usually a liability, because it drifts away from the thing
 * it was taken from; here not drifting is the entire point. The check that opens this page
 * is asking whether it can still see a fault that was really made, rather than one somebody
 * imagined afterwards, and the answer stops meaning anything the moment this file is
 * brought up to date.
 *
 * Two lines differ from the commit, both imports: the same live modules, named by a longer
 * path, because this file sits further away from them. They are deliberately left live.
 * The fault is in the indexing rather than the palette, so neither a change to `color.js`
 * nor one to `bloom.js` can make the lower half of this canvas get painted.
 */
