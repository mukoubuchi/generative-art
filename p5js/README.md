...
An arrow that is leaned on now keeps going. p5 reports a key going down once and then not again while it stays down — it guards its own handler against the repeats a browser sends — so a held arrow used to be worth a single section; and letting those repeats through instead would only have handed the speed of the spiral to whatever the reader's system settings happen to be. So the hold is timed by the sketch: 250 milliseconds before it starts repeating, a section every 120 milliseconds after that, which builds all fifteen from the first rectangle in a little under one and three quarter seconds. A tap is still exactly one section, the beat is the same on a fast machine and a slow one, and the repeat stops of its own accord at either end of the spiral rather than sitting there asking for sections that do not exist.
