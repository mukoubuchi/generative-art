...
**The mark on artworks that move**, from the orbit in `2026-05-02/tips-4.html`, where a small body circles a larger one and its own turn finishes at 80 per cent of the cycle, so it comes to rest for the last fifth before setting off again — that pause is what stops a loop from reading as a spinner. Here it is the badge itself. A mark that says an artwork moves may as well move, and this site's artworks are made of orbits and spirals. It makes one lap as the card arrives and only keeps going under the pointer: twenty-eight of the thirty-seven carry it, and twenty-eight things circling on their own is a page that will not sit still.
