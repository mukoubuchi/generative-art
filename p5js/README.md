...
The rings that lose a colour are the piece's own discovery rather than anything asked of it. Counting turns rather than arcs, they fall at 2, 3, 4, 7, 11, 15, 26, 41, 56, 97, 153, 362 — and that is exactly the list of turn counts that come nearer to landing back at the start than any turn count before them, counting the two ways round separately. Nothing connects the two calculations: one measures how far a single mark falls short of the start, the other counts how many distinct arc lengths a whole ring has. They agree anyway, because a ring with only two lengths is one where the marks have cut the circle about as evenly as that many marks can, and cutting evenly is what coming round almost exactly means. Nothing in the drawing marks those rings. They are where they are.
