...
| `kanizsa-square` | 680×680 | 1360×1360 MP4 at 30 fps | 10 seconds, three levers pulled and put back |
...
The figure is drawn for the mistake the artwork is named after: a tower and cap in flat black against a dusk gradient, four lattice sails wheeling on it, the wind itself visible as streaks that lengthen and brighten as it rises. One lit window on the tower gives the scale, and is the only thing in the picture that settles Sancho's argument.
...
That survey also gives the artwork its third lever. The share of the perimeter that is genuinely inked is the support ratio, the quantity this figure's strength is known to hang on, and the picture's own survey agrees with the ratio the module names to three decimals. The lever shrinks the discs while the square stays exactly where it is, so the sides empty from just under two thirds real to about a tenth, and the square a viewer was supplying quietly stops being supplied — with nothing that could be called an edge added or removed anywhere.

The other two levers are the ones the original had. The bites turn away from the sides, and the square dies although the same ink stands in the same places: the support ratio counts ink rather than where it points, so it does not move at all while the surveyed contour collapses below a fiftieth, and that gap between the two numbers is the illusion's whole dependence on alignment. The turn is half a bite's own span, which is as far off the sides as a bite can be got — a whole quarter turn would be too far, landing one edge where the other was, back along a side, and bringing a third of the real contour with it. Then the figure owns up: an opaque plate grows out of the middle to exactly the square the bites imply, covering each bite as it arrives, and shrinks away again. It is the one mark in the artwork that is not a wedge, and it says so in its own kind.
...
**The mark on artworks that move**, from the orbit in `2026-05-02/tips-4.html`, where a small body circles a larger one and its own turn finishes at 80 per cent of the cycle, so it comes to rest for the last fifth before setting off again — that pause is what stops a loop from reading as a spinner. Here it is the badge itself. A mark that says an artwork moves may as well move, and this site's artworks are made of orbits and spirals. It makes one lap as the card arrives and only keeps going under the pointer: seventeen of the thirty-three carry it, and seventeen things circling on their own is a page that will not sit still.
