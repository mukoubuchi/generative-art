...
Warm paper and one ink, which is the mill's palette turned over. The ground stands between the two the gallery already prints on, Toggle Color Ball's and Kanizsa Square's, because a fan is an indoor thing and the white of the original is a screen rather than a room; the blades are a single warm charcoal rather than black, since on paper black is a hole. The page and the export run one law: the clip reads closed forms of the step so any frame can be rebuilt alone, and the tests hold those forms against the law walked step by step, since nothing else would notice if the two drifted apart.
