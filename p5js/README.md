...
Möbius Band is the first artwork here that is not a port, and the first drawn through WEBGL. The band is one identity: sweep a segment around a circle while turning it at half the rate, and P(u + 2π, v) = P(u, −v) — a lap glues the strip to itself with a flip. The tests assert that identity and then its consequences one by one: the surface normal comes back negated after a lap, so the band has one side; the rim needs 4π to close, so it has one edge; and the marker riding the centre line stands at its starting point halfway through the clip the wrong way up, needing the second lap to come home. The clip is that journey — one small gold body standing on the surface normal, the stage turning twice while it laps twice, so the last frame hands back to the first.

What the traveller is made of matters more than it looks. Half a journey is a reflection through the surface, and a reflection carries a thing twice its standoff — so a mark lying on the centre line, standing off by nothing, is a mark the flip cannot move at all. That is what the marker was: a bead threaded on the line with a pin through it, and the bead was most of the gold. Between the frame where it passes a place and the frame where it passes that same place on the other face, 57.2 per cent of its silhouette was unchanged, and what did change was the thin pin. It is one body raised off the band now, and the same two frames share 5.4 per cent. The shape is a half turn of sine and cosine swept about the normal and pinched towards the tip, which leaves the surface horizontally and so touches it at a point: just above the foot the body climbs about fifty times faster than the ring curves out from under it, and every vertex of it stays on one side at every frame. A flat foot cannot do that — a disc of radius r laid on a ring of radius R sinks into it by about r²/2R — and the cone-shaped candidate that had one measured 0.159 units through the surface against the 0.132 that formula predicts, the difference being the twist. That flat-footed body is kept as the control the touching test is measured against, since a check that nothing sinks is worth only as much as the thing it can catch sinking.

The band is shaded by the artwork rather than by the renderer's lights, and every term is folded in absolute value. Which way the normal points is not a fact a one-sided surface can supply — that is the artwork's whole subject — so any model that reads the sign of the normal has to tear somewhere along the ring; |N·L| is exactly the part of the lighting the band can answer for. Three directions go into it now: two lights, and the eye. The band is glass, and glass is a surface that shows itself by how it stands to the viewer — seen face-on it lets the light through and barely marks the ground, seen edge-on it turns bright and nearly solid — so the eye enters the model on the same footing as the lights, in |N·V|, and cannot tear the band either. Measured over 120 stations of the turning stage: at its most solid moment 54.5 per cent of the surface is less than half opaque, at its clearest 93.5.

The traveller is lit by those same two lights and does the opposite with them: it clamps at zero where the band folds. The asymmetry is the argument, not an inconsistency in the lighting. A one-sided surface has no fact to offer about which way it faces, so the band is lit identically on both of the sides it does not have; the traveller is an ordinary solid with an inside, its normals mean something, and reading their sign — bright where it faces the light, dark where it faces away — is exactly what makes it read as a body rather than a disc of paint. One of the two things in this picture can answer the question and the other cannot, and the two shading models say so. Which is also why the traveller dims when the face it was lit on turns under the band, and that is not a fault to be corrected: it is the same event the artwork is about, arriving as light. The strength is capped where the gold's first channel would saturate, so every pixel of the body is that one gold multiplied — uncapped, the brightest facets came back [255, 246, 98], which is lemon. It runs from 0.44 to 1.19 times the gold by construction, and measured over the clip its mean sits between 55 and 98 per cent of the flat gold it replaced, averaging 72.

That is why the traveller is no longer drawn twice. It used to be painted once as a dim ghost with the depth test off and once solid, so that the half of the journey happening behind the band stayed readable — and that half is the half that makes the point. Glass does the same work honestly: the near sheet dims what is behind it instead of hiding it. What the picture needs in exchange is an order, since transparency has no depth buffer to fall back on. The traveller goes down first, alone in being opaque, and writes its depth; then the band's 1440 cells are sorted along the eye and painted from the back with the depth test on and the writing off; then the rim, which on glass is the brightest line in the picture, because a pane of glass is found by its edge. The sort was checked against the renderer's own answer rather than trusted: painted opaque, the sorted order and the depth buffer disagree on 0.08 per cent of the frame where the band crosses itself, and reversing the sort takes that to 0.40.
