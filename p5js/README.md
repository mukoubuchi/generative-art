...
Perspective is not symmetric, though, and leaning further makes that worse. The near half of the ring is magnified and the far half shrunk, so a ring drawn about the middle of the canvas hangs below it — far enough that the nearest disc left the bottom of the frame while a band of empty paper stood at the top. So the sketch asks the ring how far the figure it sweeps falls below its own centre, and draws it that much higher. The offset is measured rather than tuned: it follows the lean and the eye distance wherever those go, the tests hold the bound against every disc at every step of the clip, and the rendered canvas comes out with the same thirty-one pixels of paper above the figure as below.

Two more things follow from the lean, and the canvas is one of them. A ring leaned back is a third again as wide as it is tall — the lean compresses its height by the sine of itself, and nothing compresses its width — so on a square canvas its two ends had nowhere to go and ran off the sides, as they had in the original. The canvas is 800 by 600 now and nothing is cropped. The other is crowding: the nearest disc and the furthest sit at opposite ends of the ring, and whether they meet is a single comparison, because perspective magnifies how far apart they are and how large they are in the same proportion and it cancels. A disc has to be wider than the ring's own half-height — wider than the ring times the sine of the lean — and at forty-eight degrees it was not, by four pixels, which is why the two ends of the figure had come apart. The ring draws in to 160 and the discs grow to 272 across, which clears it by seventeen and puts the four of them back into one cluster.
...
Crowding them is also what made the handover show. A reader saw the switch, and it was not a fault beside the crowding but the crowding itself. Two discs have to overlap on the canvas for the four to read as a cluster — a disc's radius has to beat the ring's own half-height, over 118.9 pixels here — and at the moment two of them hand over, their circles stand 268.6 apart, so as balls they would clear each other only at 113.2 or less. There is no radius that crowds the ring and leaves the balls apart. They pass through one another, and a flat disc cannot draw two things passing through one another: it covers everything its own outline covers, so at the instant the two are equally far away the whole overlap changes hands at once. Measured on the rendered frames, the handover frame changed 16,784 pixels where the frames on either side of it changed 7,210 and 7,225.

So they are drawn as the balls they are. Two balls that pass through one another meet on a circle, and that circle is the boundary between the two colours: edge-on at the handover, so its shadow on the canvas is the straight line halfway between the two centres; turning as one draws ahead, so the shadow opens out and sweeps clear of the overlap over about a second and a third. Each disc is simply cut where a ball already painted stands in front of it. The run the cut follows is found rather than derived — rows are laid across the pair and the two surfaces are asked which is nearer, and the crossing is bisected — and a test holds that every point of it is a point where the two surfaces really do come to the same height, and that in space those points all lie on the circle the two balls share. The same frames now change 7,368, 7,423 and 7,388 pixels: the handover is a passage rather than an event.
